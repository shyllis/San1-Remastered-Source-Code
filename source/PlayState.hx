...
class PlayState extends MusicBeatState {
	...
	private var sex:FlxSprite;

	private var healthBarBG:FlxSprite;
	private var healthBar:FlxBar;
	var songInfoDiff:String = "";
	...
	override public function create() {
		...
		switch (curStage) {
			...
			case 'place':
				defaultCamZoom = 1;
				var sky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bgStuff/sky', 'san1'));
				sky.setGraphicSize(Std.int(sky.width * 1.1));
				sky.antialiasing = true;
				sky.scrollFactor.set(1, 0.9);
				sky.active = false;
				add(sky);
		
				var ground:FlxSprite = new FlxSprite(-800, 600).loadGraphic(Paths.image('bgStuff/ground', 'san1'));
			...
			case 'png':
				defaultCamZoom = 1;
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('pngBgStuff/bg', 'san1'));
		...
		sex = new FlxSprite(0, 0).makeGraphic(1280, 720, FlxColor.BLACK);
		sex.screenCenter();
		sex.alpha = 0;
		...
		info.text = '${SONG.song}' + ' - ' + songInfoDiff;
	...
	// hardcoding for new events frfr
	function blackscreen(balls:Bool) {
		add(sex);
		if (balls) FlxTween.tween(sex, {alpha: 1}, 1, {ease: FlxEase.quadOut});
		else FlxTween.tween(sex, {alpha: 0}, 1, {ease: FlxEase.quadOut});
	}
	...
	function endSong():Void {
		...
		if (isStoryMode) {
			...
			if (storyPlaylist.length <= 0) {
				FlxG.sound.playMusic(Paths.music('san1Menu', 'san1'));
	...
	override function stepHit() {
		...
		if (SONG.song.toLowerCase() == 'worst summer') {
			switch (curStep) {
				...
				case 96 | 672:
					defaultCamZoom = 0.68;
				case 112 | 164 | 272 | 418 | 640:
					defaultCamZoom = 0.75;
				case 124 | 174 | 196 | 430 | 556 | 600 | 720 | 782 | 814 | 848 | 880:
					defaultCamZoom = 0.85;
				case 140 | 180 | 210 | 288 | 334 | 366 | 480 | 576 | 608 | 736 | 896 | 1024:
					defaultCamZoom = 0.9;
				case 302 | 352 | 928:
					defaultCamZoom = 1;
				case 320 | 370 | 944:
		...
		if (SONG.song.toLowerCase() == 'saningmistake') {
			switch (curStep) {
				case 2:
					cinematicBars(true);
					defaultCamZoom = 0.7;
				case 18 | 320 | 352 | 448 | 548 | 612 | 802 | 864:
					defaultCamZoom = 0.8;
				case 32 | 78 | 100 | 124:
					defaultCamZoom = 0.75;
				case 48 | 96 | 110 | 772 | 832:
					defaultCamZoom = 0.85;
				case 64:
					cinematicBars(false);
					defaultCamZoom = 0.8;
				case 92 | 240 | 336 | 368 | 576 | 768:
					defaultCamZoom = 0.7; // h :(
				case 128 | 206 | 270 | 302 | 720:
					defaultCamZoom = 1;
				case 140 | 192 | 224 | 256 | 288 | 416 | 708:
					defaultCamZoom = 0.9;
				case 164:
					defaultCamZoom = 0.95;
				case 384:
		...
		if (SONG.song.toLowerCase() == 'sanistic') {
			switch (curStep) {
				case 96:
					defaultCamZoom = 0.78;
				case 128 | 192 | 312 | 376 | 440 | 480 | 512 | 576 | 800 | 882:
					defaultCamZoom = 0.85;
				case 142 | 210 | 288 | 316 | 352 | 380 | 400 | 448 | 526 | 594 | 814:
					defaultCamZoom = 0.95;
				case 160 | 224 | 276 | 544 | 608 | 782:
					defaultCamZoom = 0.9;
				case 4 | 174 | 242 | 418 | 558 | 626:
					defaultCamZoom = 1;
				case 256 | 320 | 384 | 850:
					defaultCamZoom = 0.8;
				case 64 | 508 | 704 | 832:
					defaultCamZoom = 0.7;
				case 640 | 896:
